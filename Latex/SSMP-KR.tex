...
A ship usually calls a sequence of ports and containers are loaded and unloaded at each port by cranes in a last-in-last-out (LILO) manner.
...
Actual examples show that containership stowage plans not only influence the income of shipping companies from transportation but also have a direct relation to the safety of ships and freights.
...
The problem is of practical significance. In the real world, a carrier/forwarder may collect a few containers in a time interval and evaluate needed space (stacks), in order to estimate available space left.
Or on the contrary, a carrier/forwarder would like to receive provisional shipment orders as many as possible while reserving enough space for regular shipment orders.
...
We have also read literature about containership stowage problems with shifts constraint.
...
As for method $loading\_equalK(c)$ and $loading\_lessK(c)$, we need to clear all sets of stacks once we have chosen a stack for each loading container at every port.
...
In order to show that the heuristic algorithm has a performance guarantee, we prove the bounds of feasible solutions to the problem.
...
After a deep thinking and analysis, we draw the conclusion that the distribution of containers in each stack accounts for this phenomenon.
...
In addition, statistical tests and sensitivity analysis can be used for our numerical experiments.
