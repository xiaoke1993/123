...
We have selected a set of values of K to find out the relationship between K and the stacks we used. 
Through this process, we may get the approximate value of K for one particular shipping to balance the flexibility and space utilization of the ship. 
...
Applying our algorithm into the real shipping management can enhance the hull space utilization and reduce the unnecessary spending in the case of meeting other constraints. 
...
\mbox{s.t.~}&\sum_{s=1}^{S} x_{is}=1, ~\forall 1\le i \le N \label{eqn:5}\\
            ...
            &\hspace{11pt}\sum_{i=1}^{N} x_{is}\le H, ~\forall 1\le s \le S	\label{eqn:7}\\
...
If they are loaded without any rehandle, the loading sequence and layout will be the following Figure \ref{fig 3:graph} and Figure \ref{fig 4:graph}:
\begin{figure}[htbp]
\centering
...
\label{fig 8:graph}
\end{figure}
Figure \ref{fig 8:graph} shows us that the greater the number of limit height is, the less stacks will be used, which is definitely right.
...
\label{fig 9:graph}
\end{figure}
Figure \ref{fig 9:graph} shows us that the comprehensive impact on the result we work out for all the basic factors considered in this paper.
