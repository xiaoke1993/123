...
%What is more, instances with different values of the number of ports, containers and heights have been tested to ensure the universality of our algorithm.
%>>>>>>> origin/master
\end{abstract}
...
World seaborne trade volumes reached 10.3 billion tons in 2016, which is shown in ~\cite{unctad2017}.
...
It is worth mentioning that our algorithm can figure out the detailed location changes of each container and output the layout status in the containership at an arbitrary time.
...
Most of the instances provide the evidence that it works better when $K$ is not equal to zero compared with the condition where $K$ is equal to zero.
However, the result of easy instances is somewhat different from the result of difficult instances.
Compared with the significant influence of different $K$ in the difficult instances, the influence of different $K$ in easy instances is not so obvious.
The same conclusion can be drawn if we focus on every single parameter while keeping other parameters unchanged.

We draw the conclusion that the distribution of containers in each stack accounts for this phenomenon after repeated experiments and analysis.
In other words, the value of parameters get smaller, the same kind of containers with the identical origin port and destination port have more chance to be placed in the same stack and less rehandles will arise.
...
The number of needed stacks is even larger when the number of allowed rehandle $K$ is not equal to zero compared the condition with zero rehandle, though the deviation is not large.
...
After all, It is the unloading strategy for blocking containers that causes this exceptional condition.
...
Comparing the layout between exceptional instances and the corresponding benchmark instances, we get the conclusion that both the distribution of containers and the algorithm result in this kind of abnormal condition.
...
It is not difficult to draw the conclusion that the number of used stacks gets smaller as the allowed rehandle becomes larger for both heuristic algorithm and random algorithm.
...
It is noteworthy that the average results in Table \ref{tab:4} include the instances with $N=5000$.
...
This algorithm is actually a process of parameters tuning, our original intention is attempting to find out a better solution to stow containers.
...
It is noteworthy that the average results in Table \ref{tab:5} include the instances with $N=5000$ and the values of two parameters in the $P\&H$ algorithm are both selected from \{0.75, 0.8\}.
It is clear that the result shows the allowed rehandles will decrease the number of used stacks as the above results in Table \ref{tab:4}.
What is more, we can draw a conclusion that the first parameter has a significant influence on the result while the second parameter makes no difference if we compare the result in each row.
...
\section{Conclusion}
\label{sec:con}
%The stowage stack minimization problem with K-rehandle constraint is aimed to find out a stowage plan that the fewest stacks are used on a containership throughout a voyage by subject to K-rehandle.
This paper deals with Stowage Stack Minimization Problem (SSMP) which minimizes the number of used stacks while regarding the number of rehandles as a constraint.
Obviously, we investigate the containership stowage problems from a new perspective since the previous research focused on how to minimize the number of rehandles.
In fact, the stowage stack minimization problem with zero rehandles is regarded as a particular case of SSMP when $K$ is set to zero.
The other way round, the problem proposed in this paper is a generalized and extended problem of SSMP-ZR considering the existence of the $K$-rehandle constraint.

In our paper, we first presented the integer programming model for the SSMP-ZR in \cite{wang2014stowage}.
...
Through experimental analysis, it can be found that it is meaningful to take the number of used stacks into consideration.
The results show the problem we put forward is of practical significance and it does reduce the number of used stacks and improve the utilization of containership.
