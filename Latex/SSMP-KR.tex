...
Second, we assume a circular route. In the real world, ship lines are all circular; no working ship stops in a port. We start from a circular route and then convert it to a equivalent linear one.
...
Since the route is circular, some containers are shipped from port $p$ to port $q$ ($p<q$) and some containers are shipped from port $p$, across port 1 and to port $q$ ($p>q$).

Given $N$ containers to be shipped, the objective of SSMP is to figure out a stowage planning so as to minimize needed stacks. The needed number of stacks is a finite number instead of an infinite number although the ship sails all the time in a circular route. This will be explained in the next subsection.
...
If overstows occur, containers above (\textit{blocking containers}) will be placed on the yard temporally when retrieving the underneath container (\textit{target container}).
...
\label{sec:h1}
A container is called a $p$-container if it is destined for port $p$.
Obviously all $p$-containers are loaded before the ship visits port $p$ and should be unloaded at port $p$.
