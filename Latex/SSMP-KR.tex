...
\begin{abstract}
%<<<<<<< HEAD
The Stowage Stack Minimization Problem (SSMP) investigates a stowage planning problem when carriers have the obligation to ship all the given containers among the ports, with the objective to utilize the fewest number of stacks on the ship. This problem has a realistic usefulness since it can help shipping forwarders estimate the shipping space of each shipper.
To the best of our knowledge, we are the first that propose SSMP in the academic community.

In this paper, we talk about SSMP with $K$-rehandles constraint. A heuristic algorithm is put forward to construct solutions. We discuss the theoretical performance guarantee of the algorithm using mathematical inequations and induction.
...
Containershipping is a kind of modern transportation mode which has a lot of advantages in speed, security, and quality compared with the break-bulk cargo ship.
Containerization is an important driver of the global economy, and container has become a mainstay of worldwide trade.
...
With the development of containershipping, more and more containers are transported by sail.
...
The problem is of practical significance. In the real world, a carrier/forwarder may collect a few containers in a time interval and evaluate needed space (stacks), in order to estimate available space left, or on the contrary, a carrier/forwarder would like to receive provisional shipment orders as many as possible while reserving enough space for regular shipment orders.
