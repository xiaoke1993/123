...
For each port $p$ along the voyage, containers with $D(c)==p$ are discharged one by one. Especially, if there exist blocking containers, they are moved on the yard temporally. 
After containers with $D(c)==p$ are discharged, containers on yard (including containers with $O(c)==p$ and blocking containers) are loaded.
...
\begin{algorithm}
  ...
  \Procname{\proc{Heuristic(I)}}
    ...
    \li         \Do
                   \If $k==K$
    \li            \Then 
                        execute method $loading\_rehandle\_equalK(c,nearport)$.
    \li            \Else If $k < K$
    \li                 execute method $loading\_rehandle\_lessK(c,nearport)$.
                   \End
                \End
    \li          Update the value of $NumofStack$. 
        \End
    \li Output the value of $NumofStack$.
...
\begin{algorithm}[htbp]
    \caption{The procedure for the method $loading\_rehandle\_equalK(c,nearport)$}
    \label{alg:2}
    \begin{codebox}
    \Procname{\proc{Method $loading\_rehandle\_equalK(c,nearport)$ }}
        \li $S_0$=currently non-full stacks whose nearport is greater than or equal to the $D(c)$.        
        \li \If $S_0 \neq \emptyset$
        \li \Then
...
\begin{algorithm}[htbp]
    \caption{The procedure for the method $loading\_rehandle\_lessK(c,nearport)$}
    \label{alg:3}
    \begin{codebox}
    \Procname{\proc{Method $loading\_rehandle\_lessK(c,nearport)$}}
        \li $S_1$=currently partial stacks whose nearport is greater than or equal to the $D(c)$.
        \li $S_2$=currently partial stacks whose nearport is smaller than the $D(c)$.
        ...
        \li             Load container $c$ to stack $s_{min}$.
        \li         \Else
        \li             Load container $c$ to the first empty stack in $S_3$. \label{heu:2a}
                    \End             
