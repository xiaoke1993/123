...
In this paper, we first propose the stowage stack minimization problem with K rehandles (SSMP-KR) based on the stowage stack minimization problem with zero rehandle.
Next, a heuristic algorithm is put forward to construct solutions to the SSMP-KR. Then, we have discussed and proved the theoretical performance guarantee of the algorithm using the math knowledge of inequation and mathematical induction.
We find out the lower bound and upper bound to make sure the solutions we get are reasonable and effective, which in turn proves the feasibility of the algorithm theoretically.

To evaluate the actual performance of our algorithm, we conduct experiments on a set of instances with practical size and compare the results when different values of K are selected, and of course zero is included.
We have selected a set of values of K to find out the relationship between K and the stacks we used in each instance.
Through this process, we can get the optimal value of K for one particular shipping to balance the flexibility and space utilization of the ship.
What¡¯s more, instances with different values of ports, containers and heights all have been experimented to make sure our algorithms have a universality.

Comparing with the zero rehandle condition and random loading condition, the results demonstrate that our heuristic approaches can generate more promising solutions if we allow some rehandling operations.
...
Applying our algorithm into the real shipping management can enhance the hull space utilization and thus to save certain speeding, which can give forwarders a theoretical reference when making decisions.
Furthermore, the novelty and practicality show the importance of this paper and it can provide a reference to the future study.
...
In view of the complexity of logistics in international trade, it can bring a great convenience if multimodal transport is used.
...
These problems have troubled and aroused the interests of both scholars and commercial shipping organization in many countries since 1970s.
With the development of container-shipping, more and more containers are transported by sail.
Over the past two decades there has been a continuous increase in demand for cost efficient containerized transportation.
Up to now, no satisfying solution has been put forward to promote the development of container transportation trade.
Thus, it makes higher research demand of the problem.
