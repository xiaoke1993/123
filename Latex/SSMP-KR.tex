...
In this paper, we talk about SSMP with $K$ rehandles. A heuristic algorithm is put forward to construct solutions. We discuss the theoretical performance guarantee of the algorithm using mathematical inequations and induction.
...
The contribution of our paper lies in four aspects.
...
Second, we assume a circular route. In the real world, ship lines are all circular; no working ship stops in a port. We start from a circular route and then convert it to a equivalent liner one.
Third, we analyze SSMP and give upper and lower bounds of SSMP. The bounds pave a way for future study.
Fourth, we propose a heuristic algorithm to solve SSMP and the algorithm has a performance guarantee.
...
\label{sec:lr}
The container handling problems can be divided into two streams based on the container handling locations occurred; one is container management at terminals, and the other is container stowage planning problems (CSP) on containerships.
...
Containers are shipped along the journey.
Since the route is circular, some containers are shipped from port $i$ to port $j$ ($i<j$) and some containers are shipped from port $i$, across port 1 and to port $j$ ($i>j$) .

Given certain containers to be shipped, the objective of SSMP is to figure out a stowage planning so as to minimize needed stacks. The needed number of stacks is a finite number instead of an infinite number although the ship sails all the time in a circular route. This will be explained in the next subsection.
In this paper, we extend SSMP with zero rehandle constraint \citep{wang2014stowage}, assuming that $K$ rehandles are allowed.

Other assumptions in this paper:
\begin{itemize}
...
If a container with far destination is placed above one with near destination, we call this \textit{overstow}. 
If overstows occur, containers above (\textit{forced containers}) will be placed on the yard temporally when retrieving the underneath container (\textit{target container}).
When moving forced containers back to the ship, there is no guarantee that they will be returned to their original slots.
A feasible solution to the SSMP is represented by a sequence of operations. Each operation indicates the number of container and the accommodating stack. Especially, the yard is term as stack 0.

\subsection{Equivalent liner route}
\begin{proposition}
An instance of circular route can be converted into an instance of liner route.
\end{proposition}

\textbf{Explanation}
Supposing that a ship starts its journey at port 1 and sequentially visits port 2, 3, ..., $P$, 1, 2, 3, ... $P$....
According to the itinerary, containers can be classified into $P*(P-1)$ types:
