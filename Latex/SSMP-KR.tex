...
\label{fig two:graph}
\end{figure}
Some slots that have power plugs are known as reefer slots. A standard ISO container is usually 20 or 40 ft long, 8 ft wide and 8.6 ft high. 
40 foot containers can be 8.6 ft high, or 9.6 ft high for high-cube containers (see Figure \ref{fig three:graph}).
...
Due to the proven NP-hardness of the problem, a so-called "Suspensory Heuristic" --- based on earlier work by \cite{avriel1993exact} --- is developed in order to solve even large problem instances.
...
The proposed approach exhibits very good performance in terms of both solution precision and computational time. 
Moreover, in the performance evaluation with real size cases, it also guarantees another very important maritime performance index, in term of handling operations/hour.
...
This so-called slot planning problem is NP-hard and often involves stowing several hundred containers. 
Using state-of-the-art constraint solvers and modeling techniques, however, we were able to solve 90\% of 236 real instances from our industrial collaborator to optimality within 1 second.
...
Similarly, we can use this method to deal with the circular route container stowage problem no matter how many circles they are.
...
when an overstowage happens, we have to unload the blocking containers, then we can unload those right ones.
...
There are 2400 sets of instances in total, which are categorized by three parameters: the number of ports $P$, which is selected from \{5, 10, 20,30\};  
the number of containers $N$, which is selected from \{50,100,200,500,1000,5000\}, and the result isn't shown in the table when $N=5000$; 
the height limit $H$ selected from \{4,7, 8, 12\}. 
...
What's more, this is applicable to the heuristic algorithm as well as the deuterogenic $P\&H$ algorithm, the simple introduced will be introduced in the next content.
