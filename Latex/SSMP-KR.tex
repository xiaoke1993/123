...
\begin{figure}[htbp]
\centering
\includegraphics[width=6.00in,height=3.00in]{Arrangement_of_cargo_space.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=4.00in,height=3.00in]{A_vessel_bay.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=4.00in,height=3.00in]{A_partially_loaded_stack.png}
...
It can be applied into the other circumstance when $O(i)>D(i)$ happens if we regard port $O(i)$ as the port 1 or in a circular route.
...
\mbox{s.t.~}& \sum_{s=1}^{S} x_{is}=1, ~\forall 1\le i \le N \label{eqn:1}\\
            & \hspace{11pt}\sum_{i=1}^{P} R_i \le K \label{eqn:2}\\
            &\hspace{24pt}\sum_{i=1}^{N} x_{is}\le H, ~\forall 1\le s \le S	\label{eqn:3}\\
            & ~~\max\hspace{6pt}\sum_{i=1}^{P} (L_i-U_i+R_i) \le S*H \label{eqn:4}\\
\end{align}
\end{small}
...
Constraints (\ref{eqn:2}) guarantee that the total number of rehandles is no greater than than $K$, the value of allowed rehandles.
Constraints (\ref{eqn:3}) ensure that the total number of containers in each stack is no greater than than $H$.
Constraints (\ref{eqn:4}) assure that at each port $p$, the number of containers in vessel does not exceed $S*H$.
...
It is straightforward that the resultant layout from such a loading constraints will be more flexible to design our algorithm and closer to the actual situation.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=4.00in,height=3.00in]{K-limit_height.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=4.00in,height=3.00in]{without rehandle.jpg}
\caption{a side view of a partially loaded stack.}
\label{fig three:graph}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=4.00in,height=3.00in]{with rehandle.jpg}
\caption{a side view of a partially loaded stack.}
\label{fig three:graph}
\end{figure}
