...
When loading containers at each port, two different methods based on our greedy rules are adopted to choose stack for each target container according to whether $k$ is equal to $K$.
The two methods are called as $loading\_lessK(c,nearport)$ and $loading\_equalK(c,nearport)$, respectively.
...
Since we have had a basic understanding of our algorithm, the tow loading methods will be introduced in the following. 
We firstly talk about the simple condition that the allowed rehandles run out, i.e. $k$ is equal to $K$.
In the $loading\_equalK(c,nearport)$ method, we consider choosing a stack from partial stacks whose $nearport$ is no less than the current loading container at first.
...
In the $loading\_lessK(c,nearport)$ method, the kinds of feasible stacks increase as a result of the existence of rehandles.
...
That's to say, we use $arraylist$ $S_1$ to store partial stacks without rehandles occurring, $arraylist$ $S_2$ to store partial stacks with rehandles occurring and $arraylist$ $S_3$ to store empty stacks.
The priority order of them is $S_1$, $S_2$, $S_3$, which represents the chosen sequence of different types of stacks.
Like the method $loading\_equalK(c,nearport)$, we choose the first one if there are more than one stacks with the smallest value of $nearport$ when we traverse each set of feasible stacks.
...
I suppose that it happens as a result of the value set of $H$, the value set of $H$ in this paper could be a little small.
...
\section{Conclusion}
\label{sec:con}
The stowage stack minimization problem with K-rehandle constraint (SSMP-KR) is aimed to find out a stowage plan that fewest stacks on a containership are 
